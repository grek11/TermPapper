    ...
    public partial class frmDomains : Form
    {
        ...
        public DialogResult AddDomain()
        {
            ...
            if (dr == DialogResult.OK)
            {
                if (newDom == "")
                {
                    MessageBox.Show("Домен не може бути без імені!");
                ...
                if (!domains.ContainsKey(newDom))
                {
                    ...
                    MessageBox.Show("Домен з таким іменем уже існує!");
            }
            return dr;
        }
        ...
        private void btnAddValue_Click(object sender, EventArgs e)
        {
            if (listBox1.SelectedIndex < 0)
            {
                MessageBox.Show("Оберіть потрібний домен");
            }
            else
        ...
        public DialogResult AddValue(ValueDomain d)
        {
            try
            {
                ...
                if (dr == DialogResult.OK)
                {
                    if (newVal == "")
                    {
                        MessageBox.Show("Значення не може бути порожнім");
                        return DialogResult.Cancel;
                    }
            ...
            {
                MessageBox.Show("Таке значення вже існує в домені");
                return DialogResult.Cancel;
            }
            catch (Exception ex)
            {
        ...
        private void btnEditDomain_Click(object sender, EventArgs e)
        {
            try
            {
                if (listBox1.SelectedIndex < 0)
                {
                    MessageBox.Show("Спочатку необхідно обрати потрібний домен");
                    return;
                }
        ...
        private void btnDelDomain_Click(object sender, EventArgs e)
        {
            ...
            string oldDom = listBox1.SelectedItem.ToString();
            if (MessageBox.Show("Видалити домен " + oldDom + "?", "Підтвердження", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                listBox1.Items.Remove(oldDom);
        ...
        private void btnEditValue_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Спочатку необхідно виділити потрібне значення");
                    return;
                }
        ...
        private void btnDelValue_Click(object sender, EventArgs e)
        {
            if (listBox1.SelectedIndex < 0 || listBox2.SelectedIndex < 0)
            {
                MessageBox.Show("Спочатку необхідно обрати потрібний домен і значення");
                return;
            }

            string oldVal = listBox2.SelectedItem.ToString();
            if (MessageBox.Show("Видалити значення " + oldVal + "?", "Підтвердження", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
