    ...
    public class Rule
    {
        ...
        public void AddReason(Fact f, int pos)
        {
            if ((pos < 0) || (pos > reasons.Count))
                throw new RuleException("Індекс знаходився поза границями списку фактів");
            if (this.Contains(f))
                throw new RuleException("Спроба добавить додати факт");
            reasons.Add(f);
        ...
        public Fact GetReason(int pos)
        {
            if ((pos < 0) || (pos > reasons.Count - 1))
                throw new RuleException("Індекс знаходився поза границями списку посилань");
            return reasons[pos];
        }
        ...
        public void Move(Fact f, int newPos)
        {
            if (this.Contains(f))
                ...
                throw new RuleException("Запитуване посилання не знайдене");
        }
        ...
        public void Remove(int pos)
        {
            if ((pos > reasons.Count - 1) || (pos < 0))
                throw new RuleException("Індекс знаходився поза границями списку посилань");
            if (reasons.Count == 1)
                throw new RuleException("Спроба видалити єдине посилання в правилі");
