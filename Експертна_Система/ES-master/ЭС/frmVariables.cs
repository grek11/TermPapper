    ...
    public partial class frmVariables : Form
    {
        ...
        public DialogResult AddVar(bool AllowQueried)
        {
            ...
            if (dr == DialogResult.OK)
            {
                ...
                if (newVar == "")
                {
                    MessageBox.Show("Змінна не може не мати ім'я!");
                    return DialogResult.Cancel;
                }
                if (vars.ContainsKey(newVar))
                {
                    MessageBox.Show("Змінна з таким ім'ям уже існує!");
                    return DialogResult.Cancel;
                }
        ...
        {
            if (comboBox1.SelectedIndex < 0)
                MessageBox.Show("Спочатку потрібно обрати домен");
            else
            {
                frmDomains f = new frmDomains(es);
        ...
        private void btnEditVar_Click(object sender, EventArgs e)
        {
            if (listBox1.SelectedIndex < 0)
            {
                MessageBox.Show("Спочатку потрібно виділити змінну");
                return;
            }
        ...
        private void btnDelVar_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Дійсно видалити змінну?", "Підтвердження", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                return;
